"""Window-closure (Manavgat) driver, round 3. Usage: wc_driver.py <tree_root> <arm> <from_stage> <to_stage> [dry]
...
def _rebasing_loads(text, *a, **k):
    ...
    if isinstance(pp, dict):
        ...
        for name, p in pp.items():
            if not str(p).startswith(LINUX_PREFIX): raise RuntimeError(f"unexpected predictor path {name}: {p}")
            pp[name] = local + str(p)[len(LINUX_PREFIX):]
    return obj
