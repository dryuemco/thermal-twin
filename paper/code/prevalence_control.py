...
for a, b, lbl in [("A_region", "A_prime_prevalence_matched", "A - A'  effect of prevalence alone"),
                  ("A_prime_prevalence_matched", "B_scar_area", "A' - B   effect of the negative pool")]:
    v = (d[a] - d[b]); se = v.std(ddof=1) / np.sqrt(len(v))
    t = stats.t.ppf(0.975, len(v) - 1) * se
    print(f"  {lbl:42s} {v.mean():+.4f}  [{v.mean()-t:+.4f}, {v.mean()+t:+.4f}]")
d.to_json(f"{STAGING}/../prevalence_control.json", orient="records", indent=1)
