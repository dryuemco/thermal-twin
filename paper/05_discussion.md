...
portability would change with weather.

(ii) **The same-geography comparison covers one region only**, and there year, seasonal phase and
population are confounded; **that confound cannot be resolved in this study area** (Section 5.2).

(iii) **All labels derive from a single burned-area product**, MCD64A1, and no second product covers
2021 and 2022 at this resolution.

(iv) **Evia remains the most imbalance-atypical population** even after the AOI extension.

(v) **Each region contributes one fire season**, so regional concept shift is confounded with event
meteorology and, since the one arm that held place fixed is withdrawn, this design cannot attribute
the shortfall to region rather than to event.

(vi) **Cross-region point estimates carry an implementation tolerance** of roughly ±0.02 to 0.03 if
...
(ix) **The interval-support counts are less stable than the point estimates behind them**: the
published split turns on a lower bound of −0.00045, so the sign pattern is stable and the counts are
not.

(x) **The five areas of interest are not comparable frames, and this cohort cannot fully repair it**
(Section 4.4). The collar radius is itself a choice and 5 km and 10 km do not agree exactly (0.608
against 0.617), and the frames were fixed upstream in `repo/`, so we can restrict them but not
extend them. Any future cohort should fix the frame by an explicit accessible-area rule
[@Barve2011] before any predictor is computed; we treat that as the main design lesson of this paper.
