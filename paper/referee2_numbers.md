...
Mean **0.00424**, SD 0.0713, range -0.1476 to 0.1325,
12 positive and 8 negative.

| Resampling unit | n | 95% interval |
|---|---|---|
| Directions (naive, ignores pairing) | 20 | [-0.0268, 0.0339] |
| Unordered pairs, cluster bootstrap | 10 | [-0.0277, 0.0355] |
| Unordered pairs, t on pair means | 10 | [-0.0339, 0.0424] |
| Regions, leave-one-out jackknife | 5 | [-0.0371, 0.0456] |
...
| bejis_2022 | 12 | 0.00207 |
| mugla_2021 | 12 | 0.00646 |
| evia_2021_extended | 12 | -0.00813 |
| montiferru_2021 | 12 | 0.00600 |
...
Recomputed 16-direction Spearman **0.8404** against the published
0.8404 (agreement to 0.0e+0).
The tie structure of the index caps the attainable value at **0.8609**, so the
observed statistic sits essentially on its own ceiling.

Pair level, which is the unit the published bootstrap resamples: 8 pairs,
Spearman **0.8660**, exact one-sided permutation
p = **0.0060** over 40,320 permutations.
...
| Fisher z on the pair-level rho | 8 | [0.4140, 0.9754] |
| Bonett-Wright on the pair-level rho | 8 | [0.2813, 0.9818] |
| Fisher z treating 16 directions as independent | 16 | [0.5909, 0.9432] |
...
|---|---|---|
| bejis_2022 ~ manavgat_2021 | 0.000 | 0.3847 |
| manavgat_2021 ~ mugla_2021 | 0.500 | 0.4356 |
| evia_2021_extended ~ manavgat_2021 | 1.000 | 0.6492 |
...
Distinct index values: 0.000, 0.500, 1.000. Tie counts: 0.000 occurs 2 times; 0.500 occurs 1 times; 1.000 occurs 5 times.
...
|---|---|---|---|---|---|
| Manavgat | 20,511 | 784 | 5439 / 235 | 237 / 28 | 60 / 12 |
...
|---|---|---|---|---|
| Manavgat | 0.0669 | `tvdi_group` | 0.0645 | 96% |
...
|---|---|---|---|---|
| Manavgat | 2.04 | 0.866 | `modis_lst_mean_celsius` 0.525 | 0.123 |
...
- **Manavgat:** modis_lst_mean_celsius is a 4-year summer-mean MODIS context layer, not a current daily MODIS observation. This is a spatial downscaling/context calibration prototype, not yet daily MODIS downscaling.
