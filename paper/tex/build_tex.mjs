...
const APPENDICES = []   // every appendix is in paper/supplementary.md (S1-S5)
  .filter(f => has(f + '.md'));
const sections = [...BODY, ...APPENDICES];

collectTableLabels(sections);   // supplement tables are collected when the supplement is built
...
const HAS_SUP = has('supplementary.md');
if (HAS_SUP) {
collectTableLabels(['supplementary']);
// References from the supplement into the manuscript (Section 4.3, Fig. 6, Table 1, the block
// equation) point at labels defined only in manuscript.tex. They are prefixed M- and resolved by
// the xr package from manuscript.aux, so the manuscript must be compiled first.
// The Markdown carries its own contents list for readers of the .md; the PDF uses \tableofcontents,
// which nests and carries page numbers.
const supMd = read('supplementary.md').replace(/^## Contents\r?\n[\s\S]*?(?=^# S1 )/m, '@@TOC@@\n\n');
const supRaw = convertBody(supMd, { src: 'supplementary' })
  .replace(/^.*@@TOC@@.*$/m, '\\setcounter{tocdepth}{2}\n\\tableofcontents\n\\clearpage');
// (labels inside verbatim, such as the superseded caption reproduced in S4.6, are text, not labels)
const supLabels = new Set([...supRaw.replace(/\\begin\{verbatim\}[\s\S]*?\\end\{verbatim\}/g, '')
  .matchAll(/\\label\{([^}]+)\}/g)].map(m => m[1]));
const supTex = supRaw.replace(/\\(eq)?ref\{([^}]+)\}/g,
  (m, eq, l) => supLabels.has(l) ? m : '\\' + (eq || '') + 'ref{M-' + l + '}');
